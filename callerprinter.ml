...
(* list the function *)
class callerPrinterClass f_name = object (self)

  val mutable isOn : bool = false
  ...
  method decl2str (n:string) = function
	...
	(str_concat ", " (List.map self#singlen2str args)) ^ 
	(if isva then ", ..."
	else "") ^ ")"
